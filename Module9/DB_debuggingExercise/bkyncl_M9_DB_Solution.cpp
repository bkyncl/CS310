...
* File Name: bkyncl_M9_DB_Solution.cpp
* Date: 5.8.24
* Description: 
*   This is the solution file to (bkyncl_M8_Exercise.cpp) with 3 bugs solved.
...
public:
    ...
    virtual void display() const{                   // FIX: Added virtual keyword to the display function so that derived classes can override it
...
public:
    ...
    Rectangle(string n, string c, double w, double h) : Shape(n,c), m_width(w), m_height(h) {}
    // FIX: Initialize base class members using the Shape constructor
    // Initializing name and color directly in the Rectangle constructor was incorrect because it 
    // resulted in the base class members being uninitialized, leading to undefined behavior.
...
public:
    ...
    double perimeter() const override {  // FIX: Rename the function getPerimeter() to perimeter() in the Circle class to match the base class function signature.
        return 2 * 3.14 * m_radius;      // When overriding a virtual function from the base class, the overridden function's name, return type, and parameters must match the base class function's signature.
    }                                    // Additionally, "perimeter()" is a pure virtual function and must be overridden by derived classes.
